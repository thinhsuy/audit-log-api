...
class PGRetrieve:
        ...
        try:
            ...
            for rec in records:
                ...
                if isinstance(raw, str) and raw:
                    try:
                        rec.meta_data = SecurityService().decrypt_field(raw)
                    except Exception:
                        rec.meta_data = None
            ...
            logger.error(f"Failed to retrieve logs: {traceback.format_exc()}") 
            return []
    ...
    async def retrieve_tenant(
        ...
        try:
            ...
            logger.error(f"Failed to retrieve tenant: {traceback.format_exc()}") 
            return None
    ...
    async def retrieve_user(
        ...
        try:
            ...
            logger.error(f"Failed to retrieve user: {traceback.format_exc()}") 
            return None
    ...
    async def retrieve_tenants(
        ...
        try:
            ...
            logger.error(f"Failed to retrieve tenants: {traceback.format_exc()}") 
            return []
    ...
    async def get_logs_stats_by_tenant(self, tenant_id: str) -> LogStats:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve stats: {traceback.format_exc()}") 
            return None
    ...
    async def get_logs_stats_alert(self, tenant_id: str, time_retention: int = 24) -> LogStats:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve stats alert: {traceback.format_exc()}") 
            return None
    ...
    async def retrieve_chat_history(
        ...
        try:
            ...
            if tenant_id:
                query = query.where(ConverationTable.tenant_id == tenant_id)

            if order_by_time:
                query = query.order_by(ConverationTable.created_at.desc())
            ...
            return [Conversation.model_validate(rec) for rec in records]
        except Exception:
            logger.error(f"Failed to retrieve conversation: {traceback.format_exc()}") 
            return []