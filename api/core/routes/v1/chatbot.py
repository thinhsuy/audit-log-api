...
async def get_chat_response(
    ...
    try:
        ...
        response: AgentResponseFormat = await AGENT.run(
            ...
            conversation=[
                Conversation(role=ChatRoleEnum.SYSTEM, content=MASTER_PROMPT)
            ] + history,
            additional_params=ToolAdditionalParams({"tenant_id": tenant_id})
        )
        ...
        if response:
            await PGCreation(db).create_bulk_conversations(
                tenant_id=tenant_id,
                conversations=[
                    Conversation(role=ChatRoleEnum.USER, content=payload.query),
                    Conversation(role=ChatRoleEnum.ASSISTANT, content=response.content)
                ]
            )
            ...
            raise HTTPException(status_code=500, detail="Cannot get the response currently")
    except Exception:
...
async def get_chat_response(
    ...
    try:
        ...
        history = await PGRetrieve(db).retrieve_chat_history(tenant_id=tenant_id)
